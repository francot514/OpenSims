...
namespace FSO.Client.GameContent
{
    ...
    public class ContentManager
    {
        ...
        static ContentManager()
        {
            ...
            if (!Directory.Exists("Houses"))
                Directory.CreateDirectory("Houses");

            XmlDocument UIGraphicsTable = new XmlDocument();
            UIGraphicsTable.Load(@"Content\uigraphics.xml");

            XmlNodeList NodeList = UIGraphicsTable.GetElementsByTagName("DefineAssetString");
            ...
            foreach (XmlNode Node in NodeList)
            {
                ...
                {
                    FileName = AppDomain.CurrentDomain.BaseDirectory + Node.Attributes["key"].Value;
                }
                else
                    FileName = AppDomain.CurrentDomain.BaseDirectory + Node.Attributes["key"].Value;
