...
namespace FSO.Client.UI.Controls
{
    ...
    public class UIMotiveDisplay : UIElement
    {
        ...
        private void DrawMotive(UISpriteBatch batch, int x, int y, int motive)
        {
            ...
            DrawLocalString(batch, MotiveNames[motive], new Vector2(x+1, y - 14), style, new Rect(0, 0, 60, 12), Framework.TextAlignment.Center); //shadow

            style.Color = temp;
            DrawLocalString(batch, MotiveNames[motive], new Vector2(x, y - 15), style, new Rect(0, 0, 60, 12), Framework.TextAlignment.Center);

            var arrowState = ArrowStates[motive];
            var arrow = TextureGenerator.GetMotiveArrow(batch.GraphicsDevice, Color.white, Color.magenta);
            if (arrowState > 0)
            {
                for (int i = 0; i < Math.Ceiling(arrowState / 60f); i++)
                    DrawLocalTexture(batch, arrow, new Rect(2, 0, 3, 5), new Vector2(x + 61 + i*4, y), new Vector2(1, 1), new Color(0x00, 0xCB, 0x39) * Math.Min(1f, arrowState/60f-i));
            } else if (arrowState < 0)
            {
                arrowState = -arrowState;
                for (int i = 0; i < Math.Ceiling(arrowState / 60f); i++)
                    DrawLocalTexture(batch, arrow, new Rect(0, 0, 3, 5), new Vector2(x-4 - i * 4, y), new Vector2(1, 1), new Color(0xD6, 0x00, 0x00) * Math.Min(1f, arrowState / 60f - i));
            }
        }
