...
namespace FSO.Files.Formats.IFF.Chunks
{
    ...
    public class SPR2Frame : ITextureProvider, IWorldTextureProvider
    {
        ...
        public Texture2D GetTexture(Graphics device)
        {
            DecodeIfRequired();
            if (PixelCache == null)
            {
                ...
                PixelCache = new Texture2D( this.Width, this.Height);
                PixelCache.SetPixelData(this.PixelData, 1);
        ...
        /// <param name="device">GraphicsDevice instance used for drawing.</param>
        /// <returns>A Texture2D instance holding the texture data.</returns>
        public Texture2D GetZTexture(Graphics device)
        {
            DecodeIfRequired();
            if (ZCache == null)
            {
                ...
                ZCache = new Texture2D( this.Width, this.Height, TextureFormat.Alpha8, true);
                ZCache.SetPixelData(this.ZBufferData, 1);
        ...
        public WorldTexture GetWorldTexture(Graphics device)
        {
        ...
        public Color[] SetData(Color[] px, byte[] zpx, Rect rect)
        {
